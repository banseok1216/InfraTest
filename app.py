...
scheduler.add_job(resource_usage_job, 'cron', minute='*/5',
                  second='10', id='resource_usage_job')
scheduler.add_job(system_job, 'cron', hour='*', minute='0',
                  second='10', id='system_job')

def send_to_rabbitmq(queue, data):
    """RabbitMQ를 이용하여 비동기로 모니터링 서버에 전달"""
    max_retries = 3
    current_time = datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')
    for attempt in range(max_retries):
        try:
            ...
            channel.basic_publish(exchange='direct', routing_key=queue, body=json.dumps(data).encode('utf-8'))
            connection.close()
